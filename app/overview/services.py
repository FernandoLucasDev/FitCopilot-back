...
def get_workspace_overview(account_id) -> dict:
    ...
    priorities = sorted(
        [item for item in scored if item[2]["status"] in {"attention", "cooling", "risk"}],
    ...
    return {
        ...
        "priorities": [
            ...
            for student, result, operational in priorities
        ],
        "aiSuggestions": [
            {
                "id": str(item.id),
        ...
        "studentsNeedingAttention": [serialize_student_list_item(student) for student, _, _ in priorities],
        "metrics": {
